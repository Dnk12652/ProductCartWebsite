...
func UserSignUp() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			log.Panic(err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": err,"m":"m"})
			return
		}
		if count > 0 {
		...
		if count > 0 {
			c.JSON(http.StatusBadRequest, gin.H{"error": "user phone or emails details exists"})
			return
		}
		...
		if  addingusererr!= nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error while registrations"})
			return
		}
		defer cancel()
		c.JSON(http.StatusCreated, "User Registrations successfully")
	}
}
